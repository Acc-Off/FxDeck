...
- **接続中サーバーのコマンド候補** — ゲーム内でワンクリックすると、サーバーの chat が持つ権限フィルタ済みのコマンド一覧を読み取り、ボタン編集の入力中に候補を表示。検索できるコマンドピッカーも開けます。
...
- サーバーに接続した状態で **設定 → 入力補助 → 接続中サーバーのコマンドを抽出** を押します。chat の UI が既に持っているサジェスト一覧 — チャットで `/` を打ったときに出る、権限（ACE）でフィルタ済みのあの一覧 — を読み取り、`commands-cache.json` にキャッシュします。
- 以後、ボタン編集のコマンド欄で入力中に候補が出ます。help と引数のヒント（`jail <id> [time]`）付きで、マクロのチェーンの途中でも効きます。欄の「コマンド一覧」ボタンからは検索できるピッカーを開けます。`+x` / `-x` のキーバインド半片や内部コマンドは、ピッカーでは既定で非表示です（トグルで表示）。
- **読み取りの仕組み**: FiveM クライアントはループバック限定の NUI デバッグポート（`127.0.0.1:13172`）を開いています。FxDeck が接続するのは**ボタンを押したときだけ**で、chat リソースがメモリ上に持つ状態を受動的に読むだけです。ゲームやサーバーへは何も送らず、常時ポーリングもしません。非公式の仕組みなのでゲームの更新で動かなくなる可能性があり、標準 chat を独自実装に差し替えたサーバーでは取得できません。候補はあくまで補助で、手動入力は常に可能です。抽出はゲーム内（セッション中）のみで、メニュー画面では取得できません。
...
| `deck-token` | アクセストークン |
| `commands-cache.json` | 入力補助用に抽出したコマンド一覧（消しても問題なし。いつでも再抽出できます） |
...
FiveM / RedM クライアントは `127.0.0.1:29200` にコンソールソケットを開いています。FxDeck はそこへの接続を維持し、コマンドを `CMND` フレームで送り、`PRNT` フレーム（コンソール出力）をスマホに中継します。このプロトコルは **非公式** で、[fxcommands](https://github.com/josh-tf/fxcommands) を参考に解析したものです。ゲームの更新で変わる可能性があります。Web 側は Kestrel の 2 リスナー（管理 API はループバック限定、デッキは全インターフェースでトークン必須）と 1 つの React SPA です。コマンド候補はもう 1 つの非公式な入口 — クライアントの CEF デバッグポート（`127.0.0.1:13172`）越しに chat NUI の状態を必要時のみ受動的に読む方法 — を使っています。
